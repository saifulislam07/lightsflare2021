                                        ...
                                        <tr>
                                            ...
                                            <?php if ($each->payment_status == 'Completed') { ?>
                                                <td style = "padding: 0 0;"><a href = "<?php echo site_url('reject-paymnet/' . $each->payment_id) ?>" onclick = "return confirm('Are you sure , you Reject the Payment ?')" style = "color: white" class = "btn btn-xs btn-warning">Reject</a>
                                                    <?php if ($each->currency_code == 'TK'): ?>
                                                        <a onclick="getData('<?php echo $each->txn_id ?>', '<?php echo $each->number ?>', '<?php echo $each->amount ?>', '<?php echo $each->reference ?>', '<?php echo $each->payment_id ?>')" data-toggle="modal" data-target="#edit-payment" style = "color: white" class = "btn btn-xs btn-danger">Edit</a></td>
                                                <?php endif; ?>
                                            <?php } else { ?>
                                                <td style = "padding: 0 0;"><a href = "<?php echo site_url('accept-paymnet/' . $each->payment_id) ?>" onclick = "return confirm('Are you sure , you Received the amount ?')" style = "color: white" class = "btn btn-xs btn-info">Received</a>
                                                    <?php if ($each->currency_code == 'TK'): ?>
                                                        <a onclick="getData('<?php echo $each->txn_id ?>', '<?php echo $each->number ?>', '<?php echo $each->amount ?>', '<?php echo $each->reference ?>', '<?php echo $each->payment_id ?>')" data-toggle="modal" data-target="#edit-payment" style = "color: white" class = "btn btn-xs btn-danger">Edit</a></td>
                                                <?php endif; ?>
                                            <?php }
