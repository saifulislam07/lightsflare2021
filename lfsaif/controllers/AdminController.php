...
class AdminController extends CI_Controller {
    ...
    public function __construct() {
        ...
        $admin_id = $this->user_id =  $this->session->userdata('admin_id');
        //  dumpVar($admin_id);
        $this->load->database();
    ...
    public function updatetrns() {
        $submission_user = $this->db->get('use_details')->result();
        foreach ($submission_user as $key => $value):
            $userPayment = $this->db->get_where('payments', array('user_id' => $value->user_id))->row();
            ...
            if ($userPayment):
                ...
                $this->db->update('submissions',$update);
               // echo $this->db->last_query();die("stop here");
            else:
                $update['disabled'] =1;
                $this->db->where('user_id',$value->user_id);
                $this->db->update('submissions',$update);
       ...
       public function delete_exhi_data($id) {
        $this->Common_model->delete_data('exhibition', 'exhibition_id', $id);
        message('Exhibition information deleted !!!');
        redirect('all-exhibition-data');
    }
 public function add_exhibition_data() {
        if (isPostBack()) {
            ...
            if (empty($_POST['name'])):
                notification(' name Validation faild !!!!');
                redirect('new-exhibition-data');
            endif;
            if (empty($_POST['category'])):
                notification(' category Validation faild !!!!');
                redirect('new-exhibition-data');
            endif;
            if (empty($_POST['type'])):
                notification(' type Validation faild !!!!');
                redirect('new-exhibition-data');
            endif;


            $inser_id = $this->Common_model->insert_data('exhibition', $data);

            $photo['image'] = $data['name'] . '_' . $_FILES['image']['name'];
            $photo['tag'] = $data['type'] . '_' . $_FILES['tag']['name'];
            if (empty($_FILES['image']['name'])) {
                notification('image Validation faild !!!!');
                redirect('new-exhibition-data');
            }

            if (!empty($_FILES['image']['name'])) {
                move_uploaded_file($_FILES['image']['tmp_name'], 'assets/admin/exhibition/' . $photo['image']);
                $this->db->where('exhibition_id ', $inser_id);
                $this->db->update('exhibition', $photo);
            }
            if (!empty($_FILES['tag']['name'])) {
                move_uploaded_file($_FILES['tag']['tmp_name'], 'assets/admin/exhibition/' . $photo['tag']);
                $this->db->where('exhibition_id ', $inser_id);
                $this->db->update('exhibition', $photo);
            }
        ...
        $data['title'] = 'Add Exhibiton Info';
        $data['mainContent'] = $this->load->view('admin/pages/add_exhibition', $data, true);
        $this->load->view('admin/include/adminMasterTemp', $data);
    }

    public function all_exhibition_data() {
        $data['title'] = 'Exhibiton Info';
        $data['allExhibitionData'] = $this->Common_model->getDataListByJoin('exhibition');
      // dumpVar($data);
        $data['mainContent'] = $this->load->view('admin/pages/exhibition', $data, true);
        $this->load->view('admin/include/adminMasterTemp', $data);
    }
    public function dashboard() {
        ...
        $data['totalBalance'] = $this->Common_model->getUserBalance($this->user_id); 
        $data['totalImages'] = $this->Common_model->countTotalSubmit($this->user_id); 
 
        $data['totalImagesByCateogrys'] = $this->Common_model->total_categori_wise_submission();


        //dumpVar($data['totalImagesByCateogrys']);
    ...
    public function all_marks() {
        $data['title'] = 'All Marks';
        $data['topPhotographs'] = $this->Common_model->topPhotographs();
        $data['mainContent'] = $this->load->view('admin/pages/all_marks', $data, true);
        $this->load->view('admin/include/adminMasterTemp', $data);
    }

    public function all_awarded() {
        $data['title'] = 'All Awarded';
        $data['allawarded'] = $this->Common_model->getAwardedDetails();
        // dumpVar($data['allawarded']);
        $data['mainContent'] = $this->load->view('admin/pages/all_awarded', $data, true);
        $this->load->view('admin/include/adminMasterTemp', $data);
    }
    ...
    public function profile_update() {
        if (isPostBack()) {
            ...
            $fileDta = explode(' ', $data['bio']);

            foreach ($fileDta as $key => $value):
                if (in_array($value, $conditionList)) {
                    $data['bio'] = 'Not allawod text';
                    // exit;
                }
            endforeach;

            //  $this->form_validation->set_rules('bio', 'Invalid text', 'trim|xss_clean');
            //   $this->form_validation->set_rules('bio', 'Some Data', 'regex_match[@#$%^&]');
            ...
            if (!empty($_FILES['image']['name'])):
                ...
                foreach ($conditionList as $key => $value):
                    ...
                    if ($result):
                        notification('Sorry you are trying to upload invalid file.');
                        redirect('my-profile');
                    endif;
                endforeach;
    ...
    public function paymentUpdate() {
        if (isPostBack()) {
            ...
            $updatedid = $this->Common_model->update_data('payments', $data, 'payment_id', $payment_id);

            if ($updatedid) {
                message('Transection details updated');
                redirect(site_url('all-detail'));
            } else {
                notification("Not Updted.!");
            }
        }
    }
    ...
    public function queries() {
        $data['title'] = 'Queries';
        $data['user_queries'] = $this->Common_model->get_data_list('contact_us', 'id', 'desc');
    ...
    public function confirm_email() {
        if (isPostBack()) {
            ...
            $this->email->from('info@lightsflare.com', 'LightsFlare Award 2020');
    ...
    public function alarm_email() {
        if (isPostBack()) {
            ...
            $user_email = $data['email'];
            $mesg = $this->load->view('admin/include/email_template_user_alarm', $data, TRUE);
            $config = Array(
                'protocol' => 'smtp',
                'smtp_host' => 'ssl://smtp.googlemail.com',
                'smtp_port' => 465,
            ...
            //  $this->email->to($user_email); // change it to yours
            $this->email->to('saiful.rana@gmail.com'); // change it to yours
            $this->email->subject('Pay Entry Fee and Confirm Your Submission');
            $this->email->message($mesg);
            $this->email->set_mailtype('html');
            if ($this->email->send()) {
                $this->Common_model->insert_data('confirmation_email', $data);
                message('Alarm email  Send !!!');
                redirect('all-submission');
            } else {
                notification('Mail Failed !');
                redirect('all-submission');
    ...
    public function reminder_email() {
        if (isPostBack()) {
            ...
            $user_email = $data['email'];
            $mesg = $this->load->view('admin/include/email_reminder_template_user', $data, TRUE);
            $config = Array(
                'protocol' => 'smtp',
                'smtp_host' => 'ssl://smtp.googlemail.com',
                'smtp_port' => 465,
            ...
            $this->email->to('saiful.rana@gmail.com'); // change it to yours
            $this->email->subject('Lightsflare Deadline Extended');
            $this->email->message($mesg);
            $this->email->set_mailtype('html');
            if ($this->email->send()) {
                // $this->Common_model->insert_data('confirmation_email', $data);
    ...
    public function delete_data($id) {
        $this->Common_model->delete_data('payments', 'payment_id', $id);
        message('Payment information deleted !!!');
        redirect('all-detail');
    }

    function contactDownload() {
        $uemail = $this->Common_model->getUserEmail();
        $filename = "Contact Contact List_" . date('d-m-Y');
        $file_ending = "xls";
        ...
        echo '<tr>';
        echo '<td style="border: 1px solid black">SL</td>';
        echo '<td style="border: 1px solid black">Customer Email</td>';
        echo '</tr>';
        foreach ($uemail as $key => $each):
            echo '<tr>';
            echo '<td style="border: 1px solid black">' . ++$key . '</td>';
            echo '<td style="border: 1px solid black">' . $each->email . '</td>';
            echo '</tr>';
        endforeach;
        echo '</table>';
    }
